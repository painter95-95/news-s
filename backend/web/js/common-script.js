jQuery(window).load(function() {
	...
	$("#smart-styles > a").on("click", function () {
		...
		if($(this).attr("id")=='magento_thm') $('body').css('background','#ffffff');else  $('body').css('background','#262a33');
...
// Tooltip
   ...
   $('.toggle-chat1').toggles({on: false});
	...
	function applyTheme (themeclass,applyClass) {
		if(themeclass=='blue_thm' || themeclass=='red_thm' || themeclass=='magento_thm' || themeclass=='green_thm' || themeclass=='magento_thm' || themeclass=='dark_theme'){
			if($('body').hasClass('magento_thm') && $.trim(applyClass)=="dark_theme")
				$('body').removeClass('magento_thm');
			if($('body').hasClass('dark_theme') && $.trim(applyClass)=="magento_thm")
				$('body').removeClass('dark_theme');
			if((themeclass == 'dark_theme' && $.trim(applyClass)!="magento_thm") || (themeclass == 'magento_thm' && $.trim(applyClass)!="dark_theme"))
				themeclass = '';

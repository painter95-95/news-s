...
Yii::setAlias('newsfolder', dirname(dirname(__DIR__)) . '/frontend/web/images/videos/');
...
class VideosController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if (!empty($model->logo)){
                ...
                $image->resize(new Box(710, 289))->save(Yii::getAlias('@newsfolder/file/'.$model->logo, ['quality' => 70]));
            }
            if ($model->save()){
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if (!empty($model->logo) || isset($model->logo)){
                ...
                @unlink(Yii::getAlias('@newsfolder/file/' . $this->findModel($id)->logo));
    ...
    {
        if ($this->findModel($id)->deleteVideo($id)){
